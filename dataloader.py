...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with open(opt.val_data, 'r') as f:
            fixed_data = json.load(f)
        self.val_data = fixed_data
        ...
        self.split_ix = {'train': [], 'val': [], 'test': [], 'attack': [], 'others': []}
        ...
        attack_idx = np.random.choice(self.split_ix['val'], 1000, replace=False)
        for ix in self.split_ix['val']:
            if ix in attack_idx:
                self.split_ix['attack'].append(ix)
            else:
                self.split_ix['others'].append(ix)
        self.split_ix['others'] = np.random.permutation(self.split_ix['others'])

        print('assigned %d images to split train' %len(self.split_ix['train']))
        print('assigned %d images to split val' %len(self.split_ix['val']))
        print('assigned %d images to split test' %len(self.split_ix['test']))
        print('--------------------- split val set ------------------------')
        print('assigned %d images to split attack' %len(self.split_ix['attack']))
        print('assigned %d images to split others' %len(self.split_ix['others']))

        self.iterators = {'train': 0, 'val': 0, 'test': 0, 'attack': 0, 'others': 0}
...
class BlobFetcher():
    ...
    def __init__(self, split, dataloader, if_shuffle=False):
        ...
        self.sampler = SubsetSampler(self.dataloader.split_ix[\
                    self.split][self.dataloader.iterators[self.split]:])

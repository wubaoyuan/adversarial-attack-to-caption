...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        ...
        output, state, attention = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
        logprobs = F.log_softmax(self.logit(output), dim=1)

        return logprobs, state, attention

    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks=None)
    ...
    def _sample(self, fc_feats, att_feats, opt={}):
        ...
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, None, opt)
        ...
        for t in range(1, self.seq_length + 1):
            ...
            logprobs, state, attention = self.get_logprobs_state(it, p_fc_feats, p_att_feats, 
        ...
        return seq, seqLogprobs, attentions
    ...
    def _inference_lvc(self, fc_feats, att_feats, s_observed):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, _ = self._prepare_feature(fc_feats, att_feats, None)
    ...
    def _inference_lai(self, fc_feats, att_feats, s_observed, slack=1.0):
        obs_idx = torch.nonzero(s_observed).squeeze(1)
        max_idx = torch.max(obs_idx)
        # if last obs_idx is end of caption, need add a <eos>
        if max_idx != len(s_observed) - 1 and s_observed[max_idx + 1] == 0:
    ...
    def _inference_m(self, fc_feats, att_feats, qs_topk, qs_inds, t_step):
        ...
        it = fc_feats.data.new(batch_size).long().zero_()
        xt = self.embed(it.view(-1))
        output, state_init, _ = self.core(xt, p_fc_feats, p_att_feats, pp_att_feats, state)
        loss = 0.
...
class Att2in2Core(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        att_res, att_feats = self.attention(state[0][-1], att_feats, p_att_feats, att_masks)

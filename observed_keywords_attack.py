...
def select_observed(s_observed, num_observed, times=3):
    # if select part of caption from the whole caption
    """
    obs_idx = torch.nonzero(s_observed[1:]).squeeze() # except the first word
    num = len(obs_idx) - num_observed
    combinations = list(itertools.combinations(obs_idx, num))
    wordpos = random.sample(combinations, times)
    """
    
    # select part of caption as hidden words [1:6]
    s_observed = observed.clone()
    obs_idx_subset = torch.nonzero(s_observed[1:6]).squeeze() # except the first word
    obs_idx = torch.nonzero(s_observed[1:]).squeeze() # except the first word
    num = len(obs_idx_subset) - num_observed
    combinations = list(itertools.combinations(obs_idx_subset, num))
    wordpos = random.sample(combinations, times)
    wordpos = [w + tuple(obs_idx[5:]) for w in wordpos]
...
def attack(opt):
    ...
    for i, info in enumerate(val_data):
        ...
        for idx, info_t in enumerate(target_data):
            tmp_obs, wordpos = select_observed(s_observed[idx, :], opt.num_observed, opt.times)
            for num, observed in enumerate(tmp_obs):
                ...
                if torch.sum(res) == 0:
                    count += 1
                prec, rec = eval_pre_rec(s_observed[idx, :], seq_f[:, 1:].squeeze(), wordpos[num], True)
                print(prec, rec)
                precision += prec
                recall += rec
